...
import economist_feed from "../economist_feed.json";
import back_channel_feed from "../back_channel_feed.json";
import { Link } from "react-router-dom";
import axios from "axios";
import Select from "react-select";
import npr from "../npr.json"
...
class Feeds extends React.Component {
  ...
  bookmarked: []
    ...
    componentDidMount() {
      this.setState({bookmarked: JSON.parse(localStorage.getItem("bookmarked"))})
    }
  ...
  render() {
    return (
      <div className="allFeed_container_outer">
          <h2 className="feeds_heading">Feeds</h2>
        <div className="filters">
          
        </div>
        <div className="allFeed_container">
        {this.state.feeds.map((e, index) => {
          return (
            <Link
              key={index}
        ...
        <h2 className="Bookmarked_Readings_h2 ">
          Bookmarked Readings <FontAwesomeIcon icon={faBookBookmark} className='faBookBookmark_icon' />
        </h2>
      </div>
        ...
        {this.state.bookmarked.length ? (
          this.state.bookmarked.map((e, index) => {
            return (
              ...
              key={index} to={{pathname: `/post/${e.id}`, state: {
                feedtitle: '',
                posts: e.reading,
              },}}>
                <div className="bookmarked_cards">
                  <div className="bookmarked_description">
                    <h4>{e.reading.title.__cdata}</h4>
                    <p>
                      <strong>Published</strong>: {e.reading.pubDate}
                    </p>
                  </div>
                </div>
              </Link>
